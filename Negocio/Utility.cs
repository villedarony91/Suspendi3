...
using System.Linq;
using System.Text;
using Suspendidos.Presentacion;
   ...
   public class Utility
    {
        ...
        public string getDepto(string code)
        {
            ConnectDB connection = new ConnectDB();
            string query = "SELECT deslarga FROM trefdepmun WHERE CODMUN = 0 AND CODDEP = " + code;
            string depto = connection.executeScalar(query).ToString();
            return depto;

        }

        public string getMun(string codDep, String codMun)
        {
            ConnectDB connection = new ConnectDB();
            string query = "SELECT deslarga FROM trefdepmun WHERE CODMUN = " +  codMun + " AND CODDEP = " + codDep;
            string mun = connection.executeScalar(query).ToString();
            return mun;
        }
